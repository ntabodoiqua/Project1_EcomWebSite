...
if (isset($_POST['confirm_cancel'])){
    ...
    $update_query_1 = "UPDATE `user_orders` SET order_status='cancelled' WHERE order_id=$order_id";
    $result_1 = mysqli_query($con, $update_query_1);
    $update_query_2 = "UPDATE `products` INNER JOIN `temp_product_sold` ON products.product_id = temp_product_sold.product_id
                        SET products.number_sold = products.number_sold - temp_product_sold.temp_sold
                        WHERE temp_product_sold.invoice_number = $invoice_number";
    $result_2 = mysqli_query($con, $update_query_2);
    $delete_query_3 = "DELETE FROM `temp_product_sold` WHERE invoice_number = $invoice_number";
    $result_3 = mysqli_query($con, $delete_query_3);
    if($result_1 && $result && $result_2 && $result_3){
