...
<form method="GET" class="mb-4">
    <div class="input-group">
        <!-- Duy trì giá trị 'view_products' trong URL để đảm bảo đúng trang -->
        <input type="hidden" name="view_products" value="true">
        <input type="text" class="form-control" name="product_keyword" placeholder="Tìm kiếm sản phẩm..." value="<?php echo isset($_GET['product_keyword']) ? $_GET['product_keyword'] : ''; ?>">
...
<table class="table table-bordered table-hover table-striped mt-5">
    ...
    <tbody class="bg-secondary text-light">
        ...
        if (isset($_GET['product_keyword'])) {
            $product_keyword = mysqli_real_escape_string($con, $_GET['product_keyword']);
            $get_products = "SELECT * FROM `products` WHERE `product_title` LIKE '%$product_keyword%'";
        } else {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $num_conlai = $rows_sold['total_number'] - $rows_sold['number_sold'];
        ?>
        <tr class="text-center">
